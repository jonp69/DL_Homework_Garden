...
class DownloadProgressWidget(QWidget):
    ...
    def __init__(self, name_resolver: Optional[FilterNameResolver] = None):
        super().__init__()
    ...
    def update_progress(self, progress: DownloadProgress) -> None:
        ...
        if progress.current_link:
            ...
            details = []
            # Resolve filter name if resolver present
            display_name = ""
            if getattr(progress.current_link, 'filter_matched_id', None) is not None and self.name_resolver:
                display_name = self.name_resolver.resolve(progress.current_link.filter_matched_id)  # type: ignore[arg-type]
            elif progress.current_link.filter_matched:
                display_name = progress.current_link.filter_matched
            if display_name:
                details.append(f"Filter: {display_name}")
        ...
        # Add log entry only when message changes to avoid duplicates from UI timer
        if progress.current_operation and progress.current_operation != self._last_log_message:
            self.add_log_entry(progress.current_operation)
            self._last_log_message = progress.current_operation
    ...
    def add_log_entry(self, message: str) -> None:
        ...
        # Limit log size (keep only last 999 lines)
